<?php
/**
 * Theme-side demo content installer.
 *
 * Seeds the same esk_* demo content the Laravel app seeds (school settings,
 * homepage CMS rows, 30 teachers, notices, events, galleries, 14 classes,
 * students, announcements, fees, subjects, exams, testimonials, payments,
 * expenses, attendances). Idempotent - guarded by the `esk_demo_seeded` option.
 *
 * Callable from theme activation and from Dashboard → System → Tools.
 *
 * @package Eskoofy
 */
 ...
 */
function esk_install_demo_content(): array {
	global $wpdb;

	if ( get_option( 'esk_demo_seeded' ) ) {
		return array( 'ok' => true, 'message' => __( 'Demo content is already installed.', 'eskoofy' ) );
	}
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$contents}'" ) === $contents ) {
		$home_rows = array(
			array(
				'page' => 'home', 'section' => 'hero', 'title' => 'Home', 'content' => null, 'image' => '',
			...
			array(
				'page' => 'home', 'section' => 'principal', 'title' => 'Principal',
				...
				'image' => '', 'meta' => null, 'sort_order' => 2,
			),
			array(
				'page' => 'home', 'section' => 'slider', 'title' => 'Recent events & activities',
	...
	$teachers = array(
		array( 'teacher1', 'Md. Abdul Rahman', 'M.Ed' ), array( 'teacher2', 'Fatima Begum', 'B.Ed' ),
		array( 'teacher3', 'Mohammad Hossain', 'M.Sc in Mathematics' ), array( 'teacher4', 'Ayesha Islam', 'M.A in English' ),
		array( 'teacher5', 'Md. Kamal Khan', 'M.Sc in Physics' ), array( 'teacher6', 'Shahinur Akter', 'M.Sc in Chemistry' ),
		array( 'teacher7', 'Md. Rafiq Haque', 'M.A in Bengali' ), array( 'teacher8', 'Nasrin Mollah', 'M.Sc in ICT' ),
		array( 'teacher9', 'Md. Jashim Sarker', 'B.Sc in Computer Science' ), array( 'teacher10', 'Shamima Chowdhury', 'M.Phil in Education' ),
		array( 'teacher11', 'Hasan Ahmed', 'PhD in Education' ), array( 'teacher12', 'Parvin Khatun', 'MBA' ),
		array( 'teacher13', 'Mizanur Siddique', 'MSS in Economics' ), array( 'teacher14', 'Shahnaz Jahan', 'BSS in Political Science' ),
		array( 'teacher15', 'Jahangir Mahmud', 'M.Sc in Biology' ), array( 'teacher16', 'Laili Pervin', 'M.Ed' ),
		array( 'teacher17', 'Tariqul Kabir', 'B.Ed' ), array( 'teacher18', 'Rokeya Nahar', 'M.Sc in Mathematics' ),
		array( 'teacher19', 'Shahidul Bhuiyan', 'M.A in English' ), array( 'teacher20', 'Mahbuba Hasan', 'M.Sc in Physics' ),
		array( 'teacher21', 'Anwar Rahman', 'M.Sc in Chemistry' ), array( 'teacher22', 'Selina Begum', 'M.A in Bengali' ),
		array( 'teacher23', 'Kawsar Hossain', 'M.Sc in ICT' ), array( 'teacher24', 'Jannatul Islam', 'B.Sc in Computer Science' ),
		array( 'teacher25', 'Riaz Khan', 'M.Phil in Education' ), array( 'teacher26', 'Maksuda Akter', 'PhD in Education' ),
		array( 'teacher27', 'Firoz Haque', 'MBA' ), array( 'teacher28', 'Tahmina Mollah', 'MSS in Economics' ),
		array( 'teacher29', 'Nazmul Sarker', 'BSS in Political Science' ), array( 'teacher30', 'Sharmin Chowdhury', 'M.Sc in Biology' ),
	);
	$teachers_table = $prefix . 'esk_teachers';
	foreach ( $teachers as $t ) {
		list( $login, $display, $qualification ) = $t;
		$existing = get_user_by( 'email', $login . '@school.com' );
		$uid      = $existing ? $existing->ID : wp_insert_user( array(
			'user_login'   => $login,
		...
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$teachers_table} WHERE user_id = %d", $uid ) ) ) {
			$wpdb->insert( $teachers_table, array( 'user_id' => $uid, 'qualification' => $qualification, 'subjects' => null ) );
		}
	}
	...
	foreach ( $notices as $n ) {
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$notices_table} WHERE title = %s", $n[0] ) ) ) {
			$wpdb->insert( $notices_table, array( 'title' => $n[0], 'content' => $n[1], 'pinned' => 0, 'created_by' => 1 ) );
		}
	}
	...
	foreach ( $events as $e ) {
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$events_table} WHERE title = %s", $e[0] ) ) ) {
			$wpdb->insert( $events_table, array(
				'created_by' => 1, 'title' => $e[0], 'description' => $e[1], 'location' => $e[2],
				'start_date' => $e[3], 'status' => 'published',
			) );
		}
	}
	...
	foreach ( $galleries as $g ) {
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$galleries_table} WHERE title = %s", $g[0] ) ) ) {
			$wpdb->insert( $galleries_table, array( 'title' => $g[0], 'image_path' => $g[1], 'category' => $g[2], 'is_published' => 1 ) );
		}
	}

	/* 7. Classes (app's full list) */
	...
	/* 8. Students (2 notable = app's "Remarkable Students") */
	$students = array(
	...
	foreach ( $students as $s ) {
		...
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$students_table} WHERE user_id = %d", $uid ) ) ) {
	...
	/* 9. Announcements (app demo) */
	$ann_table = $prefix . 'esk_announcements';
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$ann_table}'" ) === $ann_table ) {
		foreach ( array( 'Welcome to the 2026 academic year', 'New online payment option now available', 'Annual prize giving ceremony announced' ) as $atitle ) {
			if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$ann_table} WHERE title = %s", $atitle ) ) ) {
				$wpdb->insert( $ann_table, array(
					'title' => $atitle, 'body' => 'Demo announcement: ' . $atitle, 'audience' => 'all',
					'is_published' => 1, 'display_target' => 'all', 'starts_at' => current_time( 'mysql' ),
				) );
			}
		}
	}

	/* 10. Fees for Class 1 (app demo fee types) */
		...
		foreach ( $fee_types as $f ) {
			if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$fees_table} WHERE code = %s", $f[1] ) ) ) {
				$wpdb->insert( $fees_table, array(
					'name' => $f[0], 'code' => $f[1], 'class_id' => $class_id, 'amount' => $f[2],
					'fee_type' => strtolower( $f[0] ), 'frequency' => 'monthly', 'fine_amount' => 0, 'fine_type' => 'fixed',
				) );
			}
		}
	}

	/* 11. Subjects + exams for Class 1 (app demo exam types) */
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$subjects_table}'" ) === $subjects_table ) {
		$subject_ids = array();
		foreach ( array( 'Bangla', 'English', 'Mathematics' ) as $sname ) {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$exams_table}'" ) === $exams_table ) {
			$exam_types = array( array( 'Midterm', 'MID', 50 ), array( 'Final', 'FINAL', 100 ), array( 'Pre-Test', 'PRETEST', 100 ) );
			foreach ( $exam_types as $e ) {
				if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$exams_table} WHERE code = %s", $e[1] ) ) ) {
					$wpdb->insert( $exams_table, array(
						'name' => $e[0], 'code' => $e[1], 'class_id' => $class_id, 'subject_id' => $subject_ids['Mathematics'],
						'start_date' => gmdate( 'Y-m-d' ), 'end_date' => gmdate( 'Y-m-d', strtotime( '+2 days' ) ),
						'start_time' => '09:00:00', 'end_time' => '11:00:00', 'total_marks' => $e[2],
						'passing_marks' => (int) ( $e[2] * 0.33 ), 'status' => 'upcoming',
					) );
				}
			}
		}
	...
	/* 12. Testimonials (app CMS quotes) */
	...
	foreach ( array(
		...
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$testimonials_table} WHERE author_name = %s", $t[0] ) ) ) {
			$wpdb->insert( $testimonials_table, array(
				'author_name' => $t[0], 'author_designation' => $t[1], 'content' => $t[2], 'is_visible' => 1, 'sort_order' => $ti,
			) );
		}
	}

	/* 13. Payments (dashboard revenue chart + dues) */
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$pay_table}'" ) === $pay_table ) {
		for ( $i = 0; $i < 8; $i++ ) {
			...
			$wpdb->insert( $pay_table, array(
				'invoice_number' => $inv, 'amount' => $amount, 'paid_amount' => $amount, 'due_amount' => 0,
				'total_amount' => $amount, 'payment_method' => 'cash', 'payment_status' => 'completed',
				'payment_date' => gmdate( 'Y-m-d', strtotime( '-' . $i . ' months' ) ), 'created_by' => 1,
			) );
		}
		if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$pay_table} WHERE invoice_number = %s", 'INV-PEND-01' ) ) ) {
			$wpdb->insert( $pay_table, array(
				'invoice_number' => 'INV-PEND-01', 'amount' => 3000, 'paid_amount' => 1000, 'due_amount' => 2000,
				'total_amount' => 3000, 'payment_method' => 'cash', 'payment_status' => 'partial',
				'payment_date' => gmdate( 'Y-m-d' ), 'created_by' => 1,
			) );
		}
	}

	/* 14. Expenses (dashboard revenue vs expense chart) */
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$exp_table}'" ) === $exp_table ) {
		...
		foreach ( $cats as $ci => $cat ) {
			for ( $m = 0; $m < 6; $m++ ) {
				...
				if ( $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$exp_table} WHERE category = %s AND date = %s", $cat, $date ) ) ) {
					continue;
				}
				$wpdb->insert( $exp_table, array(
					'category' => $cat, 'amount' => 1500 + ( $ci * 400 ) + ( $m * 150 ), 'date' => $date, 'created_by' => 1,
				) );
			}
		}
	}

	/* 15. Attendances (dashboard today + 7-day trend) */
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$att_table}'" ) === $att_table ) {
		...
		foreach ( $student_rows as $si => $s ) {
			for ( $d = 6; $d >= 0; $d-- ) {
				...
				if ( $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$att_table} WHERE student_id = %d AND date = %s", $s->id, $date ) ) ) {
					continue;
				}
				$wpdb->insert( $att_table, array(
					'student_id' => $s->id, 'school_class_id' => $class_id, 'date' => $date,
	...
	return array( 'ok' => true, 'message' => __( 'Demo content installed.', 'eskoofy' ) );
}

/**
 * Repair the site URL: strip a stray '/client' from home/siteurl and pin them to
 * the current request root so the front page never redirects to /client/.
 *
 * @return array{ok: bool, message: string}
 */
function esk_repair_site_url(): array {
	...
	// Fall back to the stored home option host when running outside a web
	// request (e.g. wp-cli), stripping any stray /client segment.
	if ( '' === $host ) {
		$home = (string) get_option( 'home', '' );
		$parsed = parse_url( $home );
		if ( ! empty( $parsed['host'] ) ) {
			$scheme = ! empty( $parsed['scheme'] ) ? $parsed['scheme'] : $scheme;
	...
	if ( '' === $host ) {
		return array( 'ok' => false, 'message' => __( 'Could not detect the site host.', 'eskoofy' ) );
	}
