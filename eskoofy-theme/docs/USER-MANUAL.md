...
> Product: `eskoofy-theme` (WordPress theme, version 0.2.0, text domain `eskoofy`) · Audience: school administrators, teachers, accountants, librarians + site visitors · Docs language: English
...
**`/dashboard/`**, mirroring `eskoofy-app`. Its two sibling products share the same UI and
feature set on different stacks: `eskoofy-app` (Laravel) and `eskoofy-php` (raw PHP). BD/INT
...
> **Quality-gate note:** `eskoofy-theme/AGENTS.md` states "PHPCS is not a gating check (the
