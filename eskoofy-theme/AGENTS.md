...
This is the **`eskoofy-theme`** product folder inside the Eskoofy monorepo (root `AGENTS.md`).
It is a feature-equivalent port of the Laravel app (`eskoofy-app/`) as a **plugin-theme
...
- **UI parity**: the dashboard shell and the public site must look like the app
  (`eskoofy-app/`). Class naming may differ (`esk-*` vs Tailwind utilities), the rendered
  output must match.
