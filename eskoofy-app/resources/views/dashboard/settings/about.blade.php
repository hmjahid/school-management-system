    ...
    <form method="post" action="{{ route('dashboard.settings.update.about') }}" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="title_en" value="{{ old('title_en', $content->title_en ?? '') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-gray-600">{{ __('Title') }} (বাংলা)</label>
                    <input type="text" name="title_bn" value="{{ old('title_bn', $content->title_bn ?? '') }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">
                </div>
            </div>
            <label class="mt-4 inline-flex cursor-pointer items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" name="is_active" value="1" class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500" @checked(old('is_active', $content->is_active ?? true))>
                {{ __('Page is active') }}
            </label>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <textarea name="intro_en" rows="4" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">{{ old('intro_en', $values['en']['intro'] ?? '') }}</textarea>
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-gray-600">{{ __('Introduction') }} (বাংলা)</label>
                    <textarea name="intro_bn" rows="4" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">{{ old('intro_bn', $values['bn']['intro'] ?? '') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button type="button" data-cms-rep-add="about-sections" data-name="sections" class="inline-flex items-center gap-1 rounded-lg border border-brand-200 bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-700 hover:bg-brand-100">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <button type="submit" class="rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-brand-700 focus:ring-brand-500">{{ __('Save about page') }}</button>
        </div>
    </form>
