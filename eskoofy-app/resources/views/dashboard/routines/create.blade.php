...
<form method="post" action="{{ route('dashboard.routines.store') }}" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div><label class="block text-sm font-medium text-gray-700">{{ __('Routine Type') }}</label><select name="type" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">@foreach($types as $k => $v)<option value="{{ $k }}" @selected(request('type', 'class') == $k)>{{ __($v) }}</option>@endforeach</select></div>
        ...
        <div class="flex items-center gap-2 pt-6">
            ...
            <input type="checkbox" name="is_active" value="1" checked class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500">
