...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    ...
    @can('create', App\Models\StudentIdCard::class)
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('dashboard.student-id-cards.batch.create') }}" class="rounded-lg border border-brand-600 bg-white px-4 py-2 text-sm font-semibold text-brand-600 hover:bg-brand-50">{{ __('Batch generate') }}</a>
            <a href="{{ route('dashboard.student-id-cards.create') }}" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 focus:ring-brand-500">{{ __('New ID card') }}</a>
        </div>
    @endcan
</div>
        ...
        <tbody class="divide-y divide-gray-200">
            @forelse($idCards as $c)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3"><x-badge :variant="$c->status === 'active' ? 'success' : ($c->status === 'revoked' ? 'danger' : 'warning')">{{ __(ucfirst($c->status)) }}</x-badge></td>
                    <td class="px-4 py-3">
                        <a href="{{ route('dashboard.student-id-cards.show', $c) }}" class="text-brand-600 hover:text-brand-800">{{ __('View') }}</a>
                        @can('update', $c)<a href="{{ route('dashboard.student-id-cards.edit', $c) }}" class="ml-2 text-brand-600 hover:text-brand-800">{{ __('Edit') }}</a>@endcan
                        ...
                        @can('delete', $c)
                            <form method="post" action="{{ route('dashboard.student-id-cards.destroy', $c) }}" class="inline" data-confirm="{{ __('Are you sure?') }}">
                                @csrf @method('delete')
            ...
            @empty
                <tr><td colspan="7" class="px-4 py-16"><x-empty-state :title="__('No ID cards found')" :message="__('Generate ID cards for your students to see them here.')" icon="document" /></td></tr>
            @endforelse
        </tbody>
    </table>
</div>
