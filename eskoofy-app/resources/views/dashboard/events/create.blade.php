    ...
    <form method="post" action="{{ route('dashboard.events.store') }}" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2 flex items-center gap-2">
                <input id="is_virtual" name="is_virtual" type="checkbox" value="1" {{ old('is_virtual') ? 'checked' : '' }} class="size-4 rounded border-gray-300 text-brand-600">
        ...
        <div>
            <button type="submit" class="rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700">{{ __('Create event') }}</button>
        </div>
    </form>
@endsection
