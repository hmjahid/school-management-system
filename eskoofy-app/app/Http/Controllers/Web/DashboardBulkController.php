...
class DashboardBulkController extends Controller
{
    ...
    protected function exportFees(): StreamedResponse
    {
        ...
        return response()->streamDownload(function () {
            ...
            \App\Models\Fee::with('student.user')->orderBy('id')->chunk(500, function ($rows) use ($out) {
                foreach ($rows as $f) {
                    fputcsv($out, [
                        $f->student?->user?->name,
                        $f->name,
    ...
    protected function exportAttendances(): StreamedResponse
    {
        ...
        return response()->streamDownload(function () {
            ...
            \App\Models\Attendance::with(['student.user', 'class'])->orderByDesc('date')->limit(10000)->get()->each(function ($a) use ($out) {
                fputcsv($out, [
                    $a->date?->format('Y-m-d'),
                    $a->student?->user?->name,
                    $a->class?->name ?? $a->batch?->name ?? '',
                    $a->status,
                    $a->remarks,
                ]);
            });
