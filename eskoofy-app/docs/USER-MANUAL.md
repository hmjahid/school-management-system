...
> Product: `eskoofy-app` (Laravel 12 school management software) · Audience: school staff, administrators, principals, accountants, librarians, teachers, students & guardians · Docs language: English (Bengali is a product UI language, not a docs language)
...
share the same UI and feature set on different stacks: `eskoofy-php` (raw PHP) and
`eskoofy-theme` (WordPress). BD/INT are **build-time variants of one codebase**, not forks.
...
|---|---|---|
| Test suite | `cd eskoofy-app && composer test` | ✅ **927 passed** (2372 assertions) |
