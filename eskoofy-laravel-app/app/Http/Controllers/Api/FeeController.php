...
class FeeController extends Controller
{
    ...
    {
        $totalFees = Fee::count();
        $activeFees = Fee::where('status', 'active')->count();
        $totalAmount = Fee::where('status', 'active')->sum('amount');
        $totalCollected = Fee::where('status', 'active')->sum('paid_amount');
        $totalPending = max(0, $totalAmount - $totalCollected);
        $collectionRate = $totalAmount > 0 ? round(($totalCollected / $totalAmount) * 100, 1) : 0;

        $byType = Fee::where('status', 'active')
            ->selectRaw('fee_type, count(*) as count, sum(amount) as total')
            ->groupBy('fee_type')
            ->get();
