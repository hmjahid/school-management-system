...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @can('update', $teacher)
                <a href="{{ route('dashboard.teachers.edit', $teacher) }}" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 focus:ring-brand-500">{{ __('Edit') }}</a>
            @endcan
            @can('delete', $teacher)
                <form method="post" action="{{ route('dashboard.teachers.destroy', $teacher) }}" data-confirm="{{ __('Remove this teacher?') }}" data-confirm-title="{{ __('Remove teacher') }}">
                    @csrf
                    @method('DELETE')
