...
<nav class="admin-sidebar-nav flex flex-1 flex-col overflow-y-auto px-3 py-4" data-no-loading>
    ...
    <div class="space-y-0.5">
        ...
        {{-- Documents --}}
        <details class="group" @if (request()->routeIs('dashboard.admit-cards*') || request()->routeIs('dashboard.student-id-cards*') || request()->routeIs('dashboard.certificates*') || request()->routeIs('dashboard.testimonials*')) open @endif>
            <summary class="admin-nav-link cursor-pointer list-none [&::-webkit-details-marker]:hidden {{ request()->routeIs('dashboard.admit-cards*') || request()->routeIs('dashboard.student-id-cards*') || request()->routeIs('dashboard.certificates*') || request()->routeIs('dashboard.testimonials*') ? 'admin-nav-link--active' : '' }}">
                <span class="flex h-5 w-5 shrink-0 items-center justify-center opacity-80">
        ...
        @if ($isAdmin ?? auth()->user()?->hasRole('admin'))
            ...
            @can('manage_users')
                <x-admin-nav-link :href="route('dashboard.users.index')" route-is="dashboard.users*" :icon="'<svg class=\'h-5 w-5\' fill=\'currentColor\' viewBox=\'0 0 20 20\'><path d=\'M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a7 7 0 017 7H1v-1a7 7 0 015-6.7z\'/></svg>'">{{ __('dashboard.users_and_roles') }}</x-admin-nav-link>
                <x-admin-nav-link :href="route('dashboard.roles.index')" route-is="dashboard.roles*" :icon="'<svg class=\'h-5 w-5\' fill=\'currentColor\' viewBox=\'0 0 20 20\'><path fill-rule=\'evenodd\' d=\'M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z\' clip-rule=\'evenodd\'/></svg>'">{{ __('dashboard.roles') }}</x-admin-nav-link>
                <x-admin-nav-link :href="route('dashboard.permissions.index')" route-is="dashboard.permissions*" :icon="'<svg class=\'h-5 w-5\' fill=\'currentColor\' viewBox=\'0 0 20 20\'><path fill-rule=\'evenodd\' d=\'M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z\' clip-rule=\'evenodd\'/></svg>'">{{ __('dashboard.permissions') }}</x-admin-nav-link>
            @endcan
            ...
            <x-admin-nav-link :href="route('dashboard.settings.general')" route-is="dashboard.settings.*" :icon="'<svg class=\'h-5 w-5\' fill=\'currentColor\' viewBox=\'0 0 20 20\'><path fill-rule=\'evenodd\' d=\'M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z\' clip-rule=\'evenodd\'/></svg>'">{{ __('dashboard.settings') }}</x-admin-nav-link>
