...
@section('content')
    ...
    <form method="post" action="{{ route('messages.store') }}" class="space-y-6">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
            <div>
                <label for="receiver_id" class="block text-sm font-medium text-gray-700">{{ __('To') }}</label>
                <select id="receiver_id" name="receiver_id" required
                    ...
                    @foreach($receivers as $receiver)
                        <option value="{{ $receiver->id }}" @selected(old('receiver_id', $replyTo->sender_id ?? '') == $receiver->id)>
                            {{ $receiver->name }} ({{ $receiver->roles->pluck('name')->join(', ') }})
                        </option>
                    @endforeach
                </select>
                @error('receiver_id')
