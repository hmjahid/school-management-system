...
@section('content')
    <x-page-header :title="__('Generate ID cards')" :description="__('Create ID cards for every student in a class or section.')">
        <x-slot:breadcrumbs>
            <x-admin-breadcrumbs :items="[
                ...
                ['label' => __('ID cards'), 'url' => route('dashboard.student-id-cards.index')],
                ['label' => __('Batch')],
            ]" />
        </x-slot:breadcrumbs>
    </x-page-header>
    ...
    <div class="admin-card max-w-2xl">
        ...
        <div class="admin-card-body">
            <form method="post" action="{{ route('dashboard.student-id-cards.batch.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ __('Expiry date') }}</label>
                        <input type="date" name="expiry_date" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100">
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('dashboard.student-id-cards.index') }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
