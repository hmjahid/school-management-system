...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="{{ route('dashboard.testimonials.index') }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">{{ __('Back') }}</a>
</div>
@include('dashboard.documents._school-info-bar')
@include('dashboard.partials.form-errors')
<form method="post" action="{{ route('dashboard.testimonials.update', $testimonial) }}" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    @csrf @method('put')
    @php $details = $testimonial->details ?? []; @endphp
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
            ...
            <select name="student_id" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                @foreach($students as $s)
                    <option value="{{ $s->id }}" @selected(old('student_id', $testimonial->student_id) == $s->id)>{{ $s->user?->name }} ({{ $s->admission_number }})</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Type') }} *</label>
            <select name="testimonial_type" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                @foreach($types as $t)
                    <option value="{{ $t }}" @selected(old('testimonial_type', $testimonial->testimonial_type) === $t)>{{ __(ucfirst(str_replace('_', ' ', $t))) }}</option>
                @endforeach
            </select>
        </div>
        <div class="sm:col-span-2">
            <label class="block text-sm font-medium text-gray-700">{{ __('Title') }} *</label>
            <input type="text" name="name" value="{{ old('name', $testimonial->name) }}" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Issue date') }} *</label>
            <input type="date" name="issue_date" value="{{ old('issue_date', $testimonial->issue_date?->format('Y-m-d')) }}" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Status') }} *</label>
            <select name="status" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                <option value="draft" @selected(old('status', $testimonial->status) === 'draft')>{{ __('Draft') }}</option>
                <option value="issued" @selected(old('status', $testimonial->status) === 'issued')>{{ __('Issued') }}</option>
                <option value="revoked" @selected(old('status', $testimonial->status) === 'revoked')>{{ __('Revoked') }}</option>
            </select>
        </div>
        <div class="sm:col-span-2">
            <label class="block text-sm font-medium text-gray-700">{{ __('Testimonial body / content') }}</label>
            <textarea name="body" rows="4" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" placeholder="{{ __('Enter testimonial content...') }}">{{ old('body', is_array($testimonial->body) ? implode("\n", $testimonial->body) : $testimonial->body) }}</textarea>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Given by (name)') }}</label>
            <input type="text" name="author_name" value="{{ old('author_name', $testimonial->author_name) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Designation') }}</label>
            <input type="text" name="author_designation" value="{{ old('author_designation', $testimonial->author_designation) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('Rating') }}</label>
            <select name="rating" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                @for($i = 1; $i <= 5; $i++)
                    <option value="{{ $i }}" @selected(old('rating', $testimonial->rating ?? 5) == $i)>{{ $i }} {{ __('Star') }}{{ $i > 1 ? 's' : '' }}</option>
                @endfor
            </select>
        </div>
        <div class="flex items-end">
            <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                ...
                <input type="checkbox" name="is_visible" value="1" @checked(old('is_visible', $testimonial->is_visible)) class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                {{ __('Show on website') }}
            </label>
        </div>
    </div>
