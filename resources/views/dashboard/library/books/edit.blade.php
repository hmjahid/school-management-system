...
@section('title', __('dashboard.edit_book') . ' — ' . config('app.name'))
@section('content')
...
<form method="post" action="{{ route('dashboard.library.books.update', $book) }}" enctype="multipart/form-data" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
            ...
            <input name="title" value="{{ old('title', $book->title) }}" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.author') }}</label>
            <input name="author" value="{{ old('author', $book->author) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.publisher') }}</label>
            <input name="publisher" value="{{ old('publisher', $book->publisher) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.isbn') }}</label>
            <input name="isbn" value="{{ old('isbn', $book->isbn) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
        ...
        <div>
            ...
            <input name="shelf_location" value="{{ old('shelf_location', $book->shelf_location) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.quantity') }} *</label>
            <input type="number" name="quantity" value="{{ old('quantity', $book->quantity) }}" min="1" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
            <p class="mt-1 text-xs text-gray-500">{{ __('dashboard.available_quantity') }}: {{ $book->available_quantity }}</p>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.purchase_date') }}</label>
            <input type="date" name="purchase_date" value="{{ old('purchase_date', $book->purchase_date?->format('Y-m-d')) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">{{ __('dashboard.price') }}</label>
            <input type="number" step="0.01" name="price" value="{{ old('price', $book->price) }}" min="0" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        </div>
        <div>
            ...
            @if($book->cover_url)
                <p class="mt-1 text-xs text-gray-500"><img src="{{ $book->cover_url }}" alt="" class="mt-1 h-12 w-10 rounded object-cover"></p>
            @endif
        </div>
