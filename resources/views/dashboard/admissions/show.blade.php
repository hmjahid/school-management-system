...
@section('content')
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm lg:col-span-2">
            ...
            <h3 class="mt-8 text-sm font-semibold uppercase tracking-wide text-gray-500">{{ __('Payment') }}</h3>
            <dl class="mt-3 grid gap-3 text-sm sm:grid-cols-2">
                <dt class="text-gray-500">{{ __('Fee') }}</dt>
                <dd class="text-gray-900">{{ number_format((float) $admission->admission_fee, 2) }}</dd>
                <dt class="text-gray-500">{{ __('Payment number') }}</dt>
                <dd class="font-mono text-gray-900">{{ $admission->payment_number ?: '—' }}</dd>
                <dt class="text-gray-500">{{ __('Method') }}</dt>
                <dd class="text-gray-900">{{ $admission->payment_method ? strtoupper($admission->payment_method) : '—' }}</dd>
                <dt class="text-gray-500">{{ __('Transaction ID') }}</dt>
                <dd class="font-mono text-gray-900">{{ $admission->transaction_id ?: '—' }}</dd>
                ...
                @if($admission->verified_at)
                    ...
                    <dd class="text-gray-900">{{ $admission->verified_at->format('Y-m-d H:i') }}</dd>
                @endif
                @if($admission->payment_note)
                    <dt class="text-gray-500">{{ __('Note') }}</dt>
                    <dd class="text-gray-900">{{ $admission->payment_note }}</dd>
                @endif
            </dl>
