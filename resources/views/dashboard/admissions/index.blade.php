...
@section('content')
    ...
    <x-card class="mb-6 {{ $settings->is_open ? '!border-emerald-200' : '!border-amber-200' }}" :padding="true">
        <form method="post" action="{{ route('dashboard.admissions.toggle') }}" class="space-y-4">
            ...
            <details class="rounded-lg border border-slate-200 bg-slate-50/50 p-4" open>
                <summary class="cursor-pointer text-sm font-medium text-slate-700">{{ __('Payment configuration') }}</summary>
                <p class="mt-2 text-xs text-slate-500">{{ __('Configure the admission fee, the payment number (bKash/Nagad/bank), and instructions shown to applicants.') }}</p>
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-slate-600">{{ __('Admission fee') }}</label>
                        <input type="number" step="0.01" min="0" name="admission_fee" value="{{ old('admission_fee', $settings->admission_fee) }}" class="admin-input">
                    </div>
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-slate-600">{{ __('Payment number') }} <span class="font-normal text-slate-400">(bKash/Nagad/bank)</span></label>
                        <input type="text" name="payment_number" value="{{ old('payment_number', $settings->payment_number) }}" class="admin-input">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="mb-1.5 block text-xs font-semibold text-slate-600">{{ __('Payment instructions') }} <span class="font-normal text-slate-400">(EN)</span></label>
                        <textarea name="payment_instructions_en" rows="3" class="admin-input">{{ old('payment_instructions_en', $settings->payment_instructions_en) }}</textarea>
                    </div>
                    <div class="sm:col-span-2">
