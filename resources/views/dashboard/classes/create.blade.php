...
@section('content')
    <div class="mb-6 flex justify-between">
        ...
        <a href="{{ route('dashboard.classes') }}" class="text-sm font-medium text-brand-600 hover:text-brand-800">{{ __('Back to list') }}</a>
    </div>
    ...
    <form method="post" action="{{ route('dashboard.classes.store') }}" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="flex items-center gap-2 pt-6">
                ...
                <input type="checkbox" name="is_active" value="1" id="is_active" class="rounded border-gray-300 text-brand-600" @checked(old('is_active', true))>
