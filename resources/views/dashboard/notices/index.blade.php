...
@section('title', __('Notices') . ' — ' . config('app.name'))

@section('content')
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Notices') }}</h1>
            <p class="mt-1 text-sm text-gray-600">{{ __('Create and manage school notices for staff, students, and parents.') }}</p>
        </div>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($rows as $row)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a class="text-blue-600 hover:underline" href="{{ route('dashboard.notices.edit', $row) }}">{{ __('Edit') }}</a>
                            <form method="post" action="{{ route('dashboard.notices.destroy', $row) }}" class="inline" onsubmit="return confirm('{{ __('Delete this notice?') }}')">
                                @csrf
                                @method('delete')
                ...
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">{{ __('No notices yet.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
