...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="{{ route('dashboard.admit-cards.index') }}" class="text-sm font-medium text-brand-600 hover:text-brand-800">{{ __('Back') }}</a>
</div>
...
<form method="post" action="{{ route('dashboard.admit-cards.store') }}" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
    ...
    <button type="submit" class="rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 focus:ring-brand-500">{{ __('Generate') }}</button>
</form>
@endsection
