...
@section('title', __('Edit announcement') . ' — ' . config('app.name'))

@section('content')
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Edit announcement') }}</h1>
    ...
    <form method="post" action="{{ route('dashboard.announcements.update', $announcement) }}" class="space-y-6">
        @csrf
        @method('put')
