...
Verification baseline (2026-09-11): `cd eskoofy-app && composer test` → **923 passed, 2361 assertions**; `cd eskoofy-php && composer test` → **242 passed, 436 assertions**; `cd eskoofy-website && composer test` → **79 passed, 214 assertions**; `pint --test` clean on all touched files.
...
| 6.9 | Dashboard / API controller audit — first sweep | ✅ done | Repaired the systematic `exams.exam_date` → `start_date` (8 controllers: AdmitCardController, SeatPlanController, ProgressReportController, ReportController, SearchController, StudentController, ExamController x2, Api/ExamController, Api/ResultsController) and the `teacher_subject` table rename to `class_subject_teacher` (SubjectController + TeacherController x4). Form field names kept as `exam_date`; only SQL column references and INSERT/UPDATE column keys are translated. Added 4 new regression tests covering AdmitCardController, SeatPlanController, SearchController, TeacherController::show. Extended three controllers to use `DatabaseInterface` so the fake can be injected. Suite 242/436 (+4). Audit script `audit_controllers.php` removed (too noisy for a long-term tool); the targeted fixes are the high-confidence subset. |
