...
Touch **only** `eskoofy-php-app/`. Read `eskoofy-php-app/README.md` first if present,
...
`eskoofy-php-app` is the raw-PHP port of the Eskoofy school management system. Its
...
```bash
rg -n '\bSELECT\b' eskoofy-php-app/app/Controllers/ | wc -l
```
...
| **C4** | The query references a column that exists in the Laravel app's schema but is missing from the raw-PHP port (the port is incomplete). | **STOP** — this is a porting gap. Add a short note to `eskoofy-php-app/MIGRATION_GAPS.md` (new file, to be committed in Part D) and skip the controller action that depends on it. Document the route as "not navigable until gap is closed." |

### A4. Do not touch these on purpose

- `eskoofy-php-app/app/Core/bootstrap.php` — already fixed.
- `eskoofy-php-app/database/schema.sql` — the schema is the source of truth; do
...
- `eskoofy-php-app/composer.json` / `phpunit.xml` / `tests/` — the test
  suite is the contract; if a test fails after the audit, the test was
  wrong, not the code.
- `eskoofy-php-app/config/` — feature flags, no DB.
- `eskoofy-php-app/app/Gateways/` and the `LicenseManager` / `SmsManager` /
...
In `eskoofy-php-app/database/schema.sql`, the `refunds` table declares
...
docker exec -i eskoofy-mariadb \
    ...
    < eskoofy-php-app/database/schema.sql
```
...
read `DB_*` from `eskoofy-php-app/.env`.)
...
`eskoofy-php-app/MIGRATION_GAPS.md`:
...
Add `eskoofy-php-app/tests/Integration/FrontControllerTest.php` that:
...
2. Provides a fake database via a `DatabaseInterface` implementation
   ...
   `eskoofy-branding-website/tests/FakeDatabase.php`, but covering the
...
Add `eskoofy-php-app/tests/Integration/DevServerSmokeTest.php` that:
...
1. `cd eskoofy-php-app && composer test` — still 232+ passing, 0 new failures.
...
5. `docker exec ... mariadb -e "SHOW WARNINGS"` after re-importing the
   schema is empty.
6. `git grep -nE 'WHERE (news|events)\.\w+ = .published.|WHERE (news|events|notices|galleries|gallery_albums)\.\w+ = .active.' eskoofy-php-app/app/Controllers/` returns
...
8. A single commit per logical chunk — at minimum one commit per
   ...
   `fix(eskoofy-php-app): <controller>: <what>`.
...
- eskoofy-branding-website or any other product.
...
- Fixing Laravel's `eskoofy-laravel-app` queries (separate product, separate
