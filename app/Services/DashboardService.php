...
class DashboardService
{
    ...
    /**
     * Live attendance snapshot for the dashboard widget.
     * Today's present / absent / late counts and a 7-day trend.
     */
    public function attendanceStats(): array
    {
        ...
        $total = array_sum($todayCounts->toArray());
        $present = (int) ($todayCounts['present'] ?? 0) + (int) ($todayCounts['late'] ?? 0) + (int) ($todayCounts['half_day'] ?? 0);
        $rate = $total > 0 ? round(($present / $total) * 100, 1) : 0.0;

        return [
            'present_today' => (int) ($todayCounts['present'] ?? 0),
            'absent_today' => (int) ($todayCounts['absent'] ?? 0),
            'late_today' => (int) ($todayCounts['late'] ?? 0),
            'leave_today' => (int) ($todayCounts['on_leave'] ?? 0),
            'today_rate' => $rate,
            'trend' => $trend->map(fn($r) => [
                'date' => $r->date,
                'rate' => $r->total > 0 ? round(($r->present / $r->total) * 100, 1) : 0.0,
            ])->values()->all(),
        ];
    }
