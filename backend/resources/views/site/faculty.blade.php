...
@section('title', ($content->title ?? __('Faculty')) . ' — ' . ($siteSettings->school_name ?? config('app.name')))
@section('meta_description', $content->meta_description)

@section('content')
    <div class="bg-white">
        @include('site.partials.inner-hero', [
            'title' => $content->title ?? __('Faculty'),
        ...
        <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            ...
            <section class="mt-12">
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Teaching staff directory') }}</h2>
                <div class="mt-2 h-1 w-20 bg-orange-500"></div>
                @if($teachers->isEmpty())
                    <p class="mt-4 text-gray-600">{{ __('Faculty profiles will appear here once added to the system.') }}</p>
                @else
                    <ul class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($teachers as $teacher)
                            <li class="rounded-lg border border-gray-200 bg-white p-6 shadow-md transition-shadow hover:shadow-lg">
                                <p class="font-semibold text-gray-900">{{ $teacher->user?->name ?? __('Staff member') }}</p>
