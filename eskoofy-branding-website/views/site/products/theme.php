...
$seo = [
    'title'       => __('theme_page.title'),
    'description' => __('theme_page.sub'),
    'canonical'   => '/products/theme',
    'type'        => 'product',
    'schema'      => [
        \App\Services\Seo::product(['name' => __('product.theme_title'), 'description' => __('product.theme_sub'), 'url' => '/products/theme', 'price' => number_format($minPrice, 2), 'currency' => 'USD']),
        \App\Services\Seo::faq($faqItems),
    ],
    'breadcrumbs' => [['name' => __('nav.home'), 'url' => '/'], ['name' => __('product.theme_title'), 'url' => '/products/theme']],
];
?>
...
<section class="esk-hero text-white">
    <div class="relative z-10 max-w-7xl mx-auto px-4 py-16 md:py-24">
        <div class="max-w-3xl">
            <span class="inline-block text-xs uppercase tracking-widest bg-blue-500/20 text-blue-200 px-3 py-1 rounded-full border border-blue-400/30"><?= __('pricing.theme_section') ?></span>
            <h1 class="text-4xl md:text-5xl font-extrabold mt-4 leading-tight"><?= __('theme_page.title') ?></h1>
            <p class="mt-5 text-slate-300 text-lg leading-relaxed"><?= __('theme_page.sub') ?></p>
            <p class="mt-4 text-sm text-blue-200/90 max-w-xl"><?= __('theme_page.audience') ?></p>
...
<section class="max-w-7xl mx-auto px-4 py-16 md:py-20">
    <div class="text-center mb-12">
        ...
        <h2 class="text-3xl md:text-4xl font-extrabold mt-3"><?= __('theme_page.benefits_title') ?></h2>
    </div>
    <div class="grid md:grid-cols-3 gap-6">
        <?php foreach ([['b1', '#2563eb'], ['b2', '#16a34a'], ['b3', '#7c3aed'], ['b4', '#ea580c'], ['b5', '#0ea5e9'], ['b6', '#db2777']] as [$key, $color]): ?>
            <div class="bg-white rounded-2xl border border-slate-200 p-7 esk-card-hover">
                ...
                <p class="mt-4 text-slate-700 font-medium leading-snug"><?= __('theme_page.' . $key) ?></p>
            </div>
        <?php endforeach; ?>
    </div>
</section>
...
<section class="bg-white border-y border-slate-200 py-16">
    <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
            ...
            <h2 class="text-3xl md:text-4xl font-extrabold mt-3"><?= __('theme_page.run_title') ?></h2>
            <p class="text-slate-500 mt-3 max-w-2xl mx-auto"><?= __('theme_page.run_sub') ?></p>
        </div>
        <div class="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-8">
                <div class="text-blue-600 font-bold text-lg"><?= __('theme_page.run_1t') ?></div>
                <p class="mt-3 text-slate-600 leading-relaxed text-sm"><?= __('theme_page.run_1d') ?></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-8">
                <div class="text-blue-600 font-bold text-lg"><?= __('theme_page.run_2t') ?></div>
                <p class="mt-3 text-slate-600 leading-relaxed text-sm"><?= __('theme_page.run_2d') ?></p>
            </div>
        </div>
    </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 pb-16">
    <div class="text-center mb-10">
        ...
        <h2 class="text-3xl font-extrabold mt-2"><?= __('theme_page.start_title') ?></h2>
    </div>
    <div class="grid md:grid-cols-3 gap-6">
        <?php foreach (['start_1', 'start_2', 'start_3'] as $i => $k): ?>
            <div class="rounded-2xl border border-slate-200 bg-white p-7 text-center">
                ...
                <p class="mt-4 font-semibold text-slate-800"><?= __('theme_page.' . $k) ?></p>
            </div>
        <?php endforeach; ?>
    </div>
</section>
