...
<section class="esk-hero text-white">
    <div class="relative z-10 max-w-7xl mx-auto px-4 py-16 md:py-20">
        <div class="max-w-3xl">
            <span class="text-xs uppercase tracking-widest bg-blue-500/20 text-blue-200 px-3 py-1 rounded-full border border-blue-400/30"><?= __('pricing.php_section') ?></span>
            <h1 class="text-4xl md:text-5xl font-extrabold mt-4"><?= __('product.php_title') ?></h1>
            <p class="mt-4 text-slate-300 text-lg leading-relaxed"><?= __('product.php_sub') ?></p>
            <div class="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-200">
                <span>✓ <?= __('product.php_deploy_1') ?></span>
                <span>✓ <?= __('product.php_deploy_2') ?></span>
            </div>
        </div>
    </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 py-16 grid md:grid-cols-3 gap-8">
    <?php foreach ($plans as $plan): ?>
        <div class="bg-white rounded-3xl p-8 border-2 border-blue-600 esk-card-hover flex flex-col shadow-xl shadow-blue-600/10 relative">
            <span class="absolute -top-3 left-8 bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-full"><?= __('product.recommended') ?></span>
...
<section class="max-w-7xl mx-auto px-4 pb-16">
    <div class="bg-white rounded-3xl border border-slate-200 p-8">
        <h2 class="text-2xl font-bold mb-6"><?= __('product.php_features') ?></h2>
