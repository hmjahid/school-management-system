<?php $pageTitle = 'Edit Admit Card'; ?>
<?php ob_start(); ?>

<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Edit Admit Card</h1>
    <a href="/dashboard/admit-cards" class="text-gray-600 hover:text-gray-800">&larr; Back</a>
</div>

<div class="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
    <form action="/dashboard/admit-cards/<?= e($card['id']) ?>" method="POST" class="space-y-4">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <select name="exam_id" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                ...
                <option value="<?= e($exam['id']) ?>" <?= ($exam['id'] == ($card['exam_id'] ?? 0)) ? 'selected' : '' ?>><?= e($exam['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <select name="status" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500">
                <option value="issued" <?= ($card['status'] ?? '') === 'issued' ? 'selected' : '' ?>>Issued</option>
                <option value="revoked" <?= ($card['status'] ?? '') === 'revoked' ? 'selected' : '' ?>>Revoked</option>
            </select>
        </div>
