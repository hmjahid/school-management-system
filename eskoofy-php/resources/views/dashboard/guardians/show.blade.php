...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @can('update', $guardian)
                <a href="{{ route('dashboard.parents.edit', $guardian) }}" class="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700">{{ __('Edit') }}</a>
            @endcan
            @can('delete', $guardian)
                <form method="post" action="{{ route('dashboard.parents.destroy', $guardian) }}" data-confirm="{{ __('Remove this guardian?') }}" data-confirm-title="{{ __('Remove guardian') }}">
                    @csrf
                    @method('DELETE')
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <ul class="mt-3 space-y-2 text-sm">
            @forelse ($guardian->students as $st)
                <li><a href="{{ route('dashboard.students.show', $st) }}" class="font-medium text-brand-600 hover:underline">{{ $st->user?->name }}</a> — {{ $st->class?->name ?? '—' }}</li>
            @empty
