...
@section('content')
    ...
    <x-card>
        <form method="post" action="/dashboard/vehicles" class="space-y-5">
            @csrf
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('Vehicle number') }}</label>
                    <input type="text" name="number" required maxlength="64" value="{{ old('number') }}" class="admin-input">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('Type') }}</label>
                    <input type="text" name="type" maxlength="64" placeholder="Bus / Van / Mini" value="{{ old('type') }}" class="admin-input">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('Capacity') }}</label>
                    <input type="number" name="capacity" min="0" value="{{ old('capacity', 0) }}" class="admin-input">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('Driver name') }}</label>
                    <input type="text" name="driver_name" maxlength="191" value="{{ old('driver_name') }}" class="admin-input">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('Driver phone') }}</label>
                    <input type="text" name="driver_phone" maxlength="32" value="{{ old('driver_phone') }}" class="admin-input">
                </div>
