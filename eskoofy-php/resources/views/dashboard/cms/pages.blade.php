...
@section('content')
    ...
    @endphp
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand-100 bg-brand-50/60 px-4 py-3 text-sm">
        ...
        <a href="{{ route('dashboard.settings.index') }}" class="text-sm font-medium text-brand-700 hover:text-brand-900">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($pages as $p)
            ...
            <a href="{{ route('dashboard.cms.edit', ['page' => $p->page]) }}"
                class="group flex flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition hover:border-brand-300 hover:shadow-md">
                <div class="mb-2 flex items-start justify-between gap-2">
                    <h3 class="text-base font-semibold text-gray-900 group-hover:text-brand-700">{{ $label }}</h3>
                    @if ($p->is_active)
                ...
                <div class="mt-3 flex items-center justify-between text-xs">
                    ...
                    <span class="font-medium text-brand-600 group-hover:text-brand-800">{{ __('Edit') }} →</span>
                </div>
            </a>
        @empty
            <div class="col-span-full rounded-2xl border-2 border-dashed border-slate-200 p-12">
                <x-empty-state :title="__('No pages yet')" :message="__('Pages will appear here once created.')" icon="document" />
            </div>
        @endforelse
    </div>
@endsection
