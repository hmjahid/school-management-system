    ...
    <x-page-header :title="__('Edit route') . ' — ' . ($route['name'] ?? '')">
        <x-slot:breadcrumbs>
            <x-admin-breadcrumbs :items="[
                ...
                ['label' => __('Transport'), 'url' => '/dashboard/transport-routes'],
                ['label' => $route['code'] ?? ''],
            ]" />
        </x-slot:breadcrumbs>
    </x-page-header>

    <form method="post" action="/dashboard/transport-routes/{{ $route['id'] }}" class="space-y-6">
        @csrf @method('put')
        ...
        <x-card :title="__('Route details')">
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="fare" required value="{{ old('fare', $route['fare'] ?? 0) }}" class="admin-input">
                </div>
                <label class="inline-flex items-center gap-2 sm:col-span-2">
                    <input type="checkbox" name="is_active" value="1" @checked((int) old('is_active', $route['is_active'] ?? 1) === 1) class="h-4 w-4 rounded border-slate-300 text-brand-600">
        ...
        <x-card :title="__('Stops')" :padding="false">
            <div x-data="{ stops: {{ json_encode(old('stops', collect($stops ?? [])->map(fn ($s) => ['id' => $s['id'], 'name' => $s['name'], 'pickup_time' => $s['pickup_time'] ?: '', 'drop_time' => $s['drop_time'] ?: '', 'sort' => $s['sort']])->all())) }} }">
        ...
        <div class="flex justify-end gap-2">
            <x-button :href="'/dashboard/transport-routes'" variant="ghost">{{ __('Cancel') }}</x-button>
