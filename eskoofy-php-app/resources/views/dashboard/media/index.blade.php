...
@section('content')
                ...
                <div class="flex items-center gap-2">
                    <form method="get" class="relative">
                        ...
                        <input type="search" name="search" value="{{ request('search') }}" placeholder="{{ __('Search media…') }}"
                            class="w-56 rounded-lg border border-gray-300 bg-white py-2 pl-9 pr-3 text-sm shadow-sm placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">
                    </form>
                    <select name="category" form="media-filter" class="rounded-lg border border-gray-300 bg-white py-2 pl-3 pr-8 text-sm shadow-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20" onchange="this.form.submit()">
    ...
    <form method="post" action="{{ route('dashboard.media.store') }}" enctype="multipart/form-data" class="mb-8 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div>
                ...
                <input type="file" name="file" required
                    class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">
            </div>
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" placeholder="{{ __('Auto from filename') }}"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-brand-500 focus:outline-none">
            </div>
            <div>
        ...
        <div class="mt-4">
            <button type="submit" class="rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 focus:ring-brand-500">{{ __('Upload') }}</button>
        </div>
    </form>
    @endif
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="group overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="p-4">
                    ...
                    <div class="mt-3 flex items-center gap-3 text-xs">
                        <button type="button" data-copy-url="{{ $row->url() }}" class="text-brand-600 hover:underline">{{ __('Copy URL') }}</button>
                        <a href="{{ route('dashboard.media.download', $row) }}" class="text-gray-600 hover:underline">{{ __('Download') }}</a>
                        <form method="post" action="{{ route('dashboard.media.destroy', $row) }}" class="inline" data-confirm="{{ __('Delete this item?') }}">
                            @csrf
                            @method('delete')
        ...
        @empty
            <div class="col-span-full rounded-2xl border-2 border-dashed border-slate-200 p-12">
                <x-empty-state :title="__('No media yet')" :message="__('Upload your first file above.')" icon="document" />
            </div>
        @endforelse
    </div>
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        @forelse($rows as $row)
            <div class="media-select-item group relative cursor-pointer overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-400 hover:shadow-lg"
                 data-url="{{ $row->url() }}">
                <div class="relative aspect-[4/3] bg-gray-100">
                    ...
                    @endif
                    <div class="absolute inset-0 bg-brand-600/0 transition-all duration-200 group-hover:bg-brand-600/20"></div>
                    <div class="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-brand-700 shadow-md">
        ...
        @empty
            <div class="col-span-full rounded-2xl border-2 border-dashed border-slate-200 p-12">
                <x-empty-state :title="__('No media yet')" :message="__('Upload images in the media library first.')" icon="document" :cta="['label' => __('Open media library'), 'url' => route('dashboard.media.index')]" />
            </div>
        @endforelse
    </div>
