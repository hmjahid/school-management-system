{{--
  ...
  $linkClass: classes for each real <a> (e.g. text-blue-200 hover:text-white)

  Each platform only renders when both a URL is set AND the platform's
  show_* toggle is true. Default for new show_* values is true.
--}}
@php
    $s = $settings;
    $lc = $linkClass ?? 'text-current hover:opacity-90';
@endphp
@if($s?->facebook_url && $s->show_facebook)
    <a href="{{ $s->facebook_url }}" class="{{ $lc }} inline-flex" rel="noopener noreferrer" target="_blank" aria-label="Facebook">
...
@endif
@if($s?->instagram_url && $s->show_instagram)
    <a href="{{ $s->instagram_url }}" class="{{ $lc }} inline-flex" rel="noopener noreferrer" target="_blank" aria-label="Instagram">
...
@endif
@if($s?->twitter_url && $s->show_twitter)
    <a href="{{ $s->twitter_url }}" class="{{ $lc }} inline-flex" rel="noopener noreferrer" target="_blank" aria-label="X">
...
@endif
@if($s?->youtube_url && $s->show_youtube)
    <a href="{{ $s->youtube_url }}" class="{{ $lc }} inline-flex" rel="noopener noreferrer" target="_blank" aria-label="YouTube">
...
@endif
@if($s?->linkedin_url && $s->show_linkedin)
    <a href="{{ $s->linkedin_url }}" class="{{ $lc }} inline-flex" rel="noopener noreferrer" target="_blank" aria-label="LinkedIn">
