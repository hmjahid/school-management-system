...
This is the **`eskoofy-php-app`** product folder inside the Eskoofy monorepo (root `AGENTS.md`).
It is a feature-equivalent port of the Laravel app (`eskoofy-laravel-app/`) in **raw PHP with no
...
- `resources/views/` — **copied verbatim from `eskoofy-laravel-app/`**; the raw PHP app renders the
...
- **UI parity**: keep `resources/views/` byte-identical to `eskoofy-laravel-app/`. Changes to shared
  Blade templates must be made in `eskoofy-laravel-app/` first, then copied here.
...
- **Always mirror the app**: a feature that lands in `eskoofy-laravel-app/` must land here too.
...
See `eskoofy-php-app/README.md` — the canonical table lives there (and in `docs/operations/DEMO-CREDENTIALS.md`).
...
- The port must not collide with `eskoofy-laravel-app` (8000) — keep `APP_URL` in `.env` in sync
