    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" id="gallery-grid">
            <?php if (!empty($photos)): ?>
                ...
                <div class="gallery-item group relative overflow-hidden rounded-xl cursor-pointer" data-category="<?= e(slugify($photo['category'] ?? 'all')) ?>">
                    <div class="aspect-square bg-gray-200">
                        <img src="<?= e(!empty($photo['image_path']) ? '/uploads/gallery/' . $photo['image_path'] : '/uploads/gallery/' . ($photo['image'] ?? '')) ?>" alt="<?= e($photo['title'] ?? '') ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
                    </div>
